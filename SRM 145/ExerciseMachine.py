...
class ExerciseMachine:
    def getPercentages(self, time):
        hours, minutes, seconds = list(map(int, time.strip().split(':')))
        print(hours, minutes, seconds)
        total = float(hours)*3600 + float(minutes)*60 + float(seconds)
        interval = total/100
        intervals = [interval*i for i in range(1, 100) if math.floor(interval*i) == interval*i]
